...
export const Reviews = ({productId,averageRating}) => {
    ...
    useEffect(()=>{
        ...
        if(reviewAddStatus==='fulfilled'){
            toast.success("Добавлен отзыв")
        }
        else if(reviewAddStatus==='rejected'){
            toast.error("Ошибка при публикации отзыва, пожалуйста, повторите попытку позже")
        }
    ...
    useEffect(()=>{
        ...
        if(reviewDeleteStatus==='fulfilled'){
            ...
            toast.error("Ошибка при удалении отзыва, пожалуйста, повторите попытку позже")
        }
    },[reviewDeleteStatus])
    ...
    useEffect(()=>{
        ...
        if(reviewUpdateStatus==='fulfilled'){
            ...
            toast.error("Проверьте исправление ошибки, пожалуйста, повторите попытку позже")
        }
    },[reviewUpdateStatus])
  ...
  return (
        <Stack rowGap={5} alignSelf={"flex-start"}  width={is480?"90vw":is840?"25rem":'40rem'}>
            ...
            <Stack>
                <Typography gutterBottom variant='h4' fontWeight={400}>Отзывы</Typography>
                {
                    reviews?.length?(
                        <Stack rowGap={3}>
                            ...
                            <Stack rowGap={1} >
                                ...
                                <Typography variant='h6' color={'text.secondary'} >Основанный на {reviews.length} {reviews.length===1?"Отзыве":"Отзывах"}</Typography>
                            </Stack>
                            ...
                            <Stack rowGap={2}>
                                {
                                    [5,4,3,2,1].map((number)=>(
                                        <Stack flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'} columnGap={1}>
                                            <Typography sx={{whiteSpace:"nowrap"}}>{number} звезд</Typography>
                        ...
                        <Typography variant='h6' color={'text.secondary'} fontWeight={400}>{loggedInUser?.isAdmin?"В данный момент отзывов нет":"Будьте тем, кто опубликует отзыв первым"}</Typography>
                    )
            ...
            {   
                ...
                <Stack rowGap={3} position={'relative'} component={'form'} noValidate onSubmit={handleSubmit(handleAddReview)}>
                    ...
                    <TextField id='reviewTextFeild' {...register("comment",{required:true})} sx={{mt:4,width:is840?"100%":"40rem"}}  multiline rows={6} fullWidth placeholder='Напишите отзыв...'/>
                    
                    <Stack>
                        <Typography gutterBottom variant='body2'>Насколько вам понравился продукт?</Typography>
                    ...
                    <Stack flexDirection={'row'} alignSelf={'flex-end'} alignItems={'center'} columnGap={'.2rem'}>
                        <MotionConfig whileTap={{scale:1}} whileHover={{scale:1.050}}>
                            <motion.div>
                                <LoadingButton sx={{textTransform:"none",fontSize:is480?"":"1rem"}} size={is480?"small":""} loading={reviewStatus==='pending'} type='submit' variant='contained'>Добавить отзыв</LoadingButton>
                            </motion.div>
                            <motion.div>
                                <Button onClick={()=>setWriteReview(false)} color='error' size={is480?"small":""} variant='outlined' sx={{textTransform:"none",fontSize:is480?"":"1rem"}}>Закрыть</Button>
                            </motion.div>
                        </MotionConfig>
                    </Stack>
                ...
                <motion.div onClick={()=>setWriteReview(!writeReview)} whileHover={{scale:1.050}} whileTap={{scale:1}} style={{width:"fit-content"}}>
                        <Button  disableElevation size={is480?"medium":'large'} variant='contained' sx={{color:theme.palette.primary.light,textTransform:"none",fontSize:"1rem",borderRadius:'6px'}}  startIcon={<CreateIcon/>}>Написать отзыв</Button>
                </motion.div>:""
            }
        </Stack>
  )
