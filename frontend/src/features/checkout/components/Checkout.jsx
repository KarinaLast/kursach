...
export const Checkout = () => {
    ...
    useEffect(()=>{
        if(addressStatus==='fulfilled'){
            ...
            alert('Ошибка в добавлении адреса')
        }
    },[addressStatus])
    ...
    <Stack flexDirection={'row'} p={2} rowGap={10} justifyContent={'center'} flexWrap={'wrap'} mb={'5rem'} mt={2} columnGap={4} alignItems={'flex-start'}>
        ...
        <Stack rowGap={4}>
            ...
            <Stack flexDirection={'row'} columnGap={is480?0.3:1} alignItems={'center'}>
                ...
                <Typography variant='h4'>Информация о доставке</Typography>
            </Stack>
            ...
            <Stack component={'form'} noValidate rowGap={2} onSubmit={handleSubmit(handleAddAddress)}>
                    <Stack>
                        ...
                        <TextField placeholder='Дом, бизнес' {...register("type",{required:true})}/>
                    </Stack>


                    <Stack>
                        <Typography gutterBottom>Улица</Typography>
                        <TextField {...register("Улица",{required:true})}/>
                    </Stack>

                    <Stack>
                        <Typography gutterBottom>Страна</Typography>
                        <TextField {...register("страна",{required:true})}/>
                    </Stack>

                    <Stack>
                        <Typography  gutterBottom>Номер телефона</Typography>
                        <TextField type='number' {...register("номер телефона",{required:true})}/>
                    </Stack>

                    <Stack flexDirection={'row'}>
                        <Stack width={'100%'}>
                            <Typography gutterBottom>Город</Typography>
                            <TextField  {...register("город",{required:true})}/>
                        </Stack>
                        <Stack width={'100%'}>
                            <Typography gutterBottom>Область</Typography>
                            <TextField  {...register("область",{required:true})}/>
                        </Stack>
                        <Stack width={'100%'}>
                            <Typography gutterBottom>Почтовый индекс</Typography>
                            <TextField type='number' {...register("почтовый индекс",{required:true})}/>
                        </Stack>
                    </Stack>

                    <Stack flexDirection={'row'} alignSelf={'flex-end'} columnGap={1}>
                        <LoadingButton loading={status==='pending'} type='submit' variant='contained'>Добавить</LoadingButton>
                        <Button color='error' variant='outlined' onClick={()=>reset()}>Сброс</Button>
                    </Stack>
            </Stack>
            ...
            <Stack rowGap={3}>
                ...
                <Stack>
                    ...
                    <Typography variant='body2' color={'text.secondary'}>Выберите один из существующих адресов</Typography>
                </Stack>
            ...
            <Stack rowGap={3}>
                    ...
                    <Stack>
                        <Typography variant='h6'>Методы оплаты</Typography>
                        <Typography variant='body2' color={'text.secondary'}>Пожалуйста, выберите способ оплаты</Typography>
                    </Stack>
                    
                    <Stack rowGap={2}>

                        <Stack flexDirection={'row'} justifyContent={'flex-start'} alignItems={'center'}>
                            <Radio value={selectedPaymentMethod} name='paymentMethod' checked={selectedPaymentMethod==='Наличка'} onChange={()=>setSelectedPaymentMethod('COD')}/>
                            <Typography>Наличная оплата</Typography>
                        </Stack>

                        <Stack flexDirection={'row'} justifyContent={'flex-start'} alignItems={'center'}>
                            <Radio value={selectedPaymentMethod} name='paymentMethod' checked={selectedPaymentMethod==='Карта'} onChange={()=>setSelectedPaymentMethod('CARD')}/>
        ...
        <Stack  width={is900?'100%':'auto'} alignItems={is900?'flex-start':''}>
            <Typography variant='h4'>Краткое описание заказа</Typography>
            <Cart checkout={true}/>
            <LoadingButton fullWidth loading={orderStatus==='pending'} variant='contained' onClick={handleCreateOrder} size='large'>Оплатите и закажите</LoadingButton>
        </Stack>
