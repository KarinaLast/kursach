...
export const Navbar=({isProductList=false})=> {
  ...
  const settings = [
    ...
    {name:'Профиль',to:loggedInUser?.isAdmin?"/admin/profile":"/profile"},
    {name:loggedInUser?.isAdmin?'Заказы':'Мои заказы',to:loggedInUser?.isAdmin?"/admin/orders":"/orders"},
    {name:'Выйти',to:"/logout"},
  ];
    ...
    <AppBar position="sticky" sx={{backgroundColor:"white",boxShadow:"none",color:"text.primary"}}>
        <Toolbar sx={{p:1,height:"4rem",display:"flex",justifyContent:"space-around"}}>
          ...
          <Stack flexDirection={'row'} alignItems={'center'} justifyContent={'center'} columnGap={2}>
            ...
            >

              {
                ...
                <MenuItem  onClick={handleCloseUserMenu}>
                  <Typography component={Link} color={'text.primary'} sx={{textDecoration:"none"}} to="/admin/add-product" textAlign="center">Добавить новый продукт</Typography>
                </MenuItem>
            ...
            <Typography variant='h6' fontWeight={300}>{is480?`${userInfo?.name.toString().split(" ")[0]}`:`Приветствуем👋, ${userInfo?.name}`}</Typography>
