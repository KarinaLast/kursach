    ...
    <Stack width={'100%'} p={is480?0:1}>
        ...
        <Stack p={2} position={'relative'} flexDirection={'column'} rowGap={1} component={'form'} noValidate onSubmit={handleSubmit(handleUpdateAddress)}>
            ...
            {/* if the edit is true then this update from shows*/}
            {
                ...
                <>
                <Typography>Улица - {street}</Typography>
                <Typography>Почтовый индекс- {postalCode}</Typography>
                <Typography>Страна - {country}</Typography>
                <Typography>Номер телефона - {phoneNumber}</Typography>
                <Typography>Область - {state}</Typography>
            ...
            <Stack position={is480?"static":edit?"static":'absolute'} bottom={4} right={4} mt={is480?2:4} flexDirection={'row'} alignSelf={'flex-end'} columnGap={1}>
                ...
                {
                    edit?(<LoadingButton loading={status==='pending'} size='small' type='submit' variant='contained'>Сохранить изменение</LoadingButton>
                    ):(<Button size='small' onClick={()=>setEdit(true)} variant='contained'>Изменить</Button>)
                }

                {/* if edit is true then cancel button is shown instead of remove */}
                {
                    edit?(
                        <Button size='small' onClick={()=>{setEdit(false);reset()}} variant='outlined' color='error'>Закрыть</Button>
                    ):(
                        <LoadingButton loading={status==='pending'} size='small' onClick={handleRemoveAddress} variant='outlined' color='error' >Удалить</LoadingButton>
                    )
                }
            </Stack>
        </Stack>
