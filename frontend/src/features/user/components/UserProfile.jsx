...
export const UserProfile = () => {
    ...
    useEffect(()=>{
        if(addressAddStatus==='fulfilled'){
            ...
            toast.error("Ошибка при добавлении адреса, пожалуйста, повторите попытку позже")
        }
    },[addressAddStatus])
    ...
    useEffect(()=>{
        ...
        if(addressUpdateStatus==='fulfilled'){
            toast.success("Обновленный адрес")
        }
        else if(addressUpdateStatus==='rejected'){
            toast.error("Ошибка при обновлении адреса, пожалуйста, повторите попытку позже")
        }
    },[addressUpdateStatus])
    ...
    useEffect(()=>{
        ...
        if(addressDeleteStatus==='fulfilled'){
            ...
            toast.error("Произошла ошибка при удалении адреса, пожалуйста, повторите попытку позже")
        }
    },[addressDeleteStatus])
            ...
            <Stack component={is480?'':Paper} elevation={1} width={is900?'100%':"50rem"} p={2} mt={is480?0:5} rowGap={2}>
                    ...
                    <Stack justifyContent={'center'} alignItems={'center'} rowGap={3}>
                        ...
                        <Stack flexDirection={'row'} alignItems={'center'} justifyContent={'center'} columnGap={1}>
                            <Typography variant='h6' fontWeight={400}>Управление адресами</Typography>
                            <Button onClick={()=>setAddAddress(true)} size={is480?'small':""} variant='contained'>Добавить</Button>
                        </Stack>
                            ...
                            addAddress?(
                                <Stack width={'100%'} component={'form'} noValidate onSubmit={handleSubmit(handleAddAddress)} rowGap={2}>
                                        ...
                                        <Stack>
                                            ...
                                            <TextField placeholder='Дом, бизнес' {...register("тип",{required:true})}/>
                                        </Stack>
                                        ...
                                        <Stack flexDirection={'row'} alignSelf={'flex-end'} columnGap={is480?1:2}>
                                            <LoadingButton loading={status==='pending'} type='submit' size={is480?"small":""} variant='contained'>Добавить</LoadingButton>
                                            <Button color='error' onClick={()=>setAddAddress(false)} variant={is480?"outlined":"text"} size={is480?"small":""} >Отмена</Button>
                                        </Stack>
                                </Stack>
                            ):('')
                        }
                        ...
                        <Stack width={'100%'} rowGap={2}>
                            {
                                addresses.length>0?(
                                    ...
                                    <Typography textAlign={'center'} mt={2} variant='body2'>У вас нет добавленных адресов</Typography>
                                )
                            }      
                        </Stack>
