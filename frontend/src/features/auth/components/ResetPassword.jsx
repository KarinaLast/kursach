        ...
        <Stack>
            ...
            <Stack component={Paper} elevation={2}>
                <Stack component={'form'} width={is500?"95vw":'30rem'} p={'1rem'} rowGap={'1rem'} noValidate onSubmit={handleSubmit(handleResetPassword)}>
                        ...
                        <Stack rowGap={'.3rem'}>
                            <Typography variant='h4' fontWeight={600}>Сброс пароля</Typography>
                            <Typography color={'GrayText'}>Пожалуйста, введите и подтвердите новый пароль</Typography>
                        </Stack>
                        ...
                        <Stack rowGap={'.5rem'}>
                            <MotionConfig whileHover={{y:-2}}>
                                ...
                                <motion.div>
                                    <TextField type='password' fullWidth sx={{mt:1}} {...register("password",{required:"Требуется ввести пароль",pattern:{value:/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/gm,message:`не менее 8 символов, должны содержать как минимум 1 заглавную букву, 1 строчную букву и 1 цифру, могут содержать специальные символы`}})} placeholder='New Password'/>
                                    {errors.password && <FormHelperText sx={{mt:1}} error>{errors.password.message}</FormHelperText>}
                                </motion.div>
                                
                                <motion.div>
                                    <TextField type='password' fullWidth sx={{mt:1}} {...register("confirmPassword",{required:"Требуется ввести пароль повторно",validate:(value,formValues)=>value===formValues.password || "Пароли не совпадают"})} placeholder='Повторить пароль'/>
                        ...
                        <motion.div whileHover={{scale:1.020}} whileTap={{scale:1}}>
                            <LoadingButton sx={{height:"2.5rem"}} fullWidth loading={status==='pending'} type='submit' variant='contained'>Сброс пароля</LoadingButton>
                        </motion.div>
                </Stack>
            </Stack>
