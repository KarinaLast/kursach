...
export const Wishlist = () => {
  ...
  useEffect(()=>{
    if(wishlistItemUpdateStatus==='fulfilled'){
      toast.success("Обновлен избранное")
    }
  ...
  useEffect(()=>{
    if(wishlistFetchStatus==='rejected'){
      toast.error("Ошибка при выборе списка желаний, пожалуйста, повторите попытку позже")
    }
  },[wishlistFetchStatus])
    ...
    <Stack justifyContent={'flex-start'} mt={is480?3:5} mb={'14rem'} alignItems={'center'}>
        ...
        <Stack width={is1130?"auto":'70rem'} rowGap={is480?2:4}>
            ...
            <Stack alignSelf={'flex-start'} flexDirection={'row'} columnGap={1} justifyContent={'center'} alignItems={'center'}>
                ...
                <Typography variant='h4' fontWeight={500}>Твои избранные</Typography>
            </Stack>
...

              {
                !wishlistFetchStatus==='pending' && wishlistItems?.length===0?(
                  ...
                  <Stack minHeight={'60vh'} width={is642?'auto':'40rem'} justifySelf={'center'}  alignSelf={'center'} justifyContent={'center'} alignItems={'center'}>
                    ...
                    <Typography variant='h6' fontWeight={300}>У вас нет товаров в вашем списке желаний</Typography>
                  </Stack>
                ):
                // wishlist grid
                      ...
                      <Stack component={is480?"":Paper} elevation={1} >
                        ...
                        <Stack paddingLeft={2} paddingRight={2} paddingBottom={2}>
                          ...
                          <Stack flexDirection={'row'} alignItems={'center'}>
                            <Typography variant='h6' fontWeight={400}>Записи</Typography>
                            ...
                            editIndex===index?(
                              ...
                              <Stack rowGap={2}>
                                ...
                                <Stack flexDirection={'row'} alignSelf={'flex-end'} columnGap={1}>
                                    <Button onClick={()=>handleNoteUpdate(item._id)} size='small' variant='contained'>Обновить</Button>
                                    <Button onClick={()=>setEditIndex(-1)} size='small' variant='outlined' color='error'>Отмена</Button>
                                </Stack>

                              </Stack>
                            ):
                            <Box>
                              <Typography sx={{wordWrap:"break-word",color:item.note?'text.primary':'GrayText'}}>{item.note?item.note:"Добавьте пользовательскую заметку здесь"}</Typography>
                            </Box>
                          }

                          {
                            cartItems.some((cartItem)=>cartItem.product._id===item.product._id)?
                            <Button sx={{mt:4}} size='small' variant='outlined' component={Link} to={'/cart'}>Уже в корзине</Button>:<Button sx={{mt:4}} size='small' onClick={()=>handleAddToCart(item.product._id)} variant='outlined'>Добавить в корзину</Button>
                          }
